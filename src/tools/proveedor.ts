...
export const leer_solicitud = {
  ...
  args: {
    caso: z.string().describe("Nombre de la carpeta del caso en reto-01/casos/ (ej: co-industrias-delta)")
  },
  async execute(args: { caso: string }, ctx?: { directory?: string; sessionId?: string }) {
    ...
    try {
      ...
      logToolExecution(
        ...
        true,
        `Solicitud leída para ${solicitudData.cliente} (${solicitudData.pais}) - Formato ${solicitudData.formato}`
      );
...
export const mapear_campos = {
  ...
  args: {
    caso: z.string().describe("Nombre de la carpeta del caso en reto-01/casos/"),
    campos: z.array(z.any()).describe("Lista de campos devueltos por leer_solicitud (etiquetas u objetos con etiqueta)")
  },
  async execute(
    args: { caso: string; campos: any[] },
    ...
    try {
      ...
      for (const item of args.campos) {
        ...
        let claveMaestro: string | undefined = glosario[etiqueta];
        ...
        const esTaxId =
          claveMaestro === "nit" ||
          /^(NIT|RUC|RTN|Identificación tributaria|Número de identificación fiscal)$/i.test(
            etiqueta.trim()
          );
...
export const generar_formulario = {
  ...
  args: {
    caso: z.string().describe("Nombre de la carpeta del caso en reto-01/casos/"),
    mapeo: z.object({
      llenos: z.array(z.any()),
      faltantes: z.array(z.any()).optional(),
      requiere_confirmacion: z.array(z.any()).optional()
    }).describe("Resultado obtenido previamente de mapear_campos")
  },
  async execute(
    args: { caso: string; mapeo: { llenos: any[]; faltantes?: any[]; requiere_confirmacion?: any[] } },
    ctx?: { directory?: string; sessionId?: string }
  ) {
    const rootDir = ctx?.directory || process.cwd();
    try {
      ...
      if (formato === "xlsx") {
        ...
        const sheetsMap: { [sheetName: string]: any[] } = {};

        for (const item of todosCampos) {
          const hojaName = item.hoja || "Hoja1";
          if (!sheetsMap[hojaName]) sheetsMap[hojaName] = [];
...
export const armar_paquete = {
  ...
  async execute(args: { caso: string }, ctx?: { directory?: string; sessionId?: string }) {
    ...
    try {
      ...
      const solicitud = JSON.parse(fs.readFileSync(solicitudPath, "utf-8"));
      const soportesExigidos: string[] = fs.existsSync(soportesExigidosPath)
        ? JSON.parse(fs.readFileSync(soportesExigidosPath, "utf-8"))
        : [];
      const indexSoportes: any[] = fs.existsSync(indexSoportesPath)
...
export const simular_envio = {
  ...
  args: {
    caso: z.string().describe("Nombre de la carpeta del caso en reto-01/casos/"),
    confirmado: z.boolean().describe("Confirmación explícita del usuario para enviar el paquete")
  },
  async execute(args: { caso: string; confirmado: boolean }, ctx?: { directory?: string; sessionId?: string }) {
    const rootDir = ctx?.directory || process.cwd();
    try {
      if (!args.confirmado) {
